...
function x264_encode() {
    ...
    ffmpeg -i ${input_video} -an -c:v libx264 -preset medium -b:v ${bitrate}k -maxrate $((bitrate * 2))k -bufsize $((bitrate * 4))k -x264opts "keyint=120:min-keyint=120:scenecut=0:no-scenecut" -pass 1 -r 30 -vf scale=w=${width}:h=${height}:flags=${method} output_${height}p.mp4
