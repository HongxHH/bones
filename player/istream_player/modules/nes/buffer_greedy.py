...
class BufferGreedyController(Module, NESController):
    ...
    def __init__(self):
        ...
        self.enhance_safety = 1 # 增强安全系数，后面会把真实增强时间除以该数以“放大”所需时间或留安全余地。
    ...
    def _greedy_control(self, action_down):
        ...
        self.enhance_time = self.enhance_time * 1000 / self.enhance_safety # 把 enhance_time 从秒转换为毫秒并除以 enhance_safety（例如 0.9），相当于把时间放大（例如真实耗时若是 900ms，除以 0.9 变为 1000ms），为增强操作留出安全裕量。

        best_quality = 0
        best_action = 0
        # 只有当增强所需时间能在单段时间 seg_time 内完成时，才考虑该增强等级（确保不会跨段过久地占用资源）。
