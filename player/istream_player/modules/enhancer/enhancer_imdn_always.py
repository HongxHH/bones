...
class IMDNEnhancerAlways(Module, Enhancer, PlayerEventListener):
    ...
    SAFE_FACTOR_MIN = 1.0
    SAFE_FACTOR_MAX = 1.3
    SAFE_FACTOR_INITIAL = 1.1
    SAFE_FACTOR_INCREASE_STEP = 0.1
    SAFE_FACTOR_DECREASE_STEP = 0.05
    # 历史记录大小
    SAFE_FACTOR_HISTORY_SIZE = 5
    ...
    def __init__(self):
        ...
        # 提前检测的检查间隔（帧数）
        self._check_interval_frames = self.CHECK_INTERVAL_FRAMES  #
        # 是否使用提前检测
        ...
        # 动态阈值安全因子（用于应对波动）
        self._threshold_safe_factor = self.SAFE_FACTOR_INITIAL  # 初始值1.1，提供10%的安全余量
        # 安全因子更新参数
        self._safe_factor_min = self.SAFE_FACTOR_MIN  # 安全因子最小值
        self._safe_factor_max = self.SAFE_FACTOR_MAX  # 安全因子最大值
        self._safe_factor_increase_step = self.SAFE_FACTOR_INCREASE_STEP  # 增加步长
        self._safe_factor_decrease_step = self.SAFE_FACTOR_DECREASE_STEP  # 减少步长
        # 安全因子更新历史记录（用于判断趋势）
        self._safe_factor_history: List[Dict[str, Any]] = []  # 记录最近N次的结果
        self._safe_factor_history_size = self.SAFE_FACTOR_HISTORY_SIZE  # 历史记录大小
        ...
        while self._is_ready:
            ...
            for as_idx in segments:
                ...
                while True:
                    ...
                    # 如果进入快速完成模式，直接使用解码器进行实时解码，避免内存拷贝
                    if fast_complete:
    ...
    def _update_dynamic_threshold(self):
        ...
        self.log.info(
            ...
            f"update use time:{time.perf_counter() - current_time:.5f}"
        )
